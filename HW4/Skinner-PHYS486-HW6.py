...
# Relaxation method. Initial conditions depend on keyword 'method'.
def relax(gridsize, duration, weight, method='random'):
    '''
    ...
    # Set initial conditions based on keyword.
    if method == 'constant':
        # Populate all elements with value == weight
        ...
        # Populate with random values on the interval [0,weight)
...
def iterative_error():
    ...
    errors = [relax(25, x, 0)[2] for x in range(1, 501)]
...
def spatial_error():
    ...
    errors = [np.sum(relax(x, 40000000, 0)[0][(int(x/2)-7):(int(x/2)+8),(int(x/2)-7):(int(x/2)+8)]-prime_grid) for x in range(17,max_size,2)]
...
def initial_error():
    ...
    iterations = [relax(25, 40000, 2*x, method='random')[1] for x in weight]
    plt.plot(weight,iterations, 'k', alpha=0.2, label='Random Start')
    for i in range(9):
        iterations = [relax(25, 40000, 2*x)[1] for x in weight]
        plt.plot(weight,iterations, 'k', alpha=0.2)

    iterations = [relax(25, 40000, x, method='constant')[1] for x in weight]
    plt.plot(weight, iterations, label='Uniform Start')
    plt.plot(0.4127083942863038, relax(25, 40000, 0,  method='interpolate')[1], '.', label='Interpolated Start')
    plt.legend()
    plt.xlim(0,1)
    plt.xlabel('Mean Value of Elements in Relaxation Region', fontsize=16)
...
gridsize = 25
duration = 2000
grid, iterations, error = relax(gridsize, duration, 0.355, method='constant')

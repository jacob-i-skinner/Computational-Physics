...
def accelerator(r, t):
    ...
    return venus_accel + sol_accel, cent_accel
...
swarm = np.array([L4 + np.array([1e6,0]), L4 + np.array([-1e6,0]), L4 + np.array([0,-1e6]), L4 + np.array([0,1e6])])
...
x = np.linspace(-R,R,1080)
y = np.linspace(-R,R,1080)
grid = np.zeros((1080,1080))
for i in range(1080):
    for j in range(1080):
        grav, cent = accelerator(np.array([x[i], y[j]]), 0)
        grid[i,j] = np.linalg.norm(grav) - np.linalg.norm(cent)

plt.pcolormesh(grid, cmap='bwr')
plt.show()